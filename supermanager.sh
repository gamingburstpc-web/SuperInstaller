...
# =====================================================
# Function: Fix a stuck/crashed VM
# =====================================================
fix_stuck_vm() {
    local vm_name=$1
    ...
    print_status "INFO" "Running stuck-VM diagnostics for: $vm_name"
    echo "--------------------------------------------------"

    # 1. Find and kill any orphaned/stale qemu process still holding the disk.
    local stale_pids
    stale_pids=$(pgrep -f "qemu-system-x86_64.*$IMG_FILE" || true)

    if [[ -n "$stale_pids" ]]; then
        print_status "WARN" "Found leftover QEMU process(es) still holding this VM's disk: $stale_pids"
        read -p "$(print_status "INPUT" "Kill them now? (Y/n): ")" -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Nn]$ ]]; then
            kill -9 $stale_pids 2>/dev/null || true
            sleep 1
            print_status "SUCCESS" "Stale process(es) terminated."
        fi
    else
        print_status "INFO" "No leftover QEMU process found for this VM. Good."
    fi

    # 2. Verify the disk image isn't corrupted / locked.
    print_status "INFO" "Checking disk image integrity: $IMG_FILE"
    if qemu-img check "$IMG_FILE" 2>&1 | tee /tmp/imgcheck.log | grep -qi "No errors"; then
        print_status "SUCCESS" "Disk image is healthy."
    else
        if grep -qi "shared.*lock\|another process" /tmp/imgcheck.log; then
            print_status "WARN" "Disk is still locked by another process. Re-run this option after closing other terminals/VMs."
        else
            print_status "WARN" "qemu-img reported an issue:"
            cat /tmp/imgcheck.log
        fi
    fi
    ...
    if ! setup_vm_image; then
        print_status "ERROR" "Failed to regenerate seed image. Check disk space with: df -h \$HOME"
        return 1
    fi

    echo "--------------------------------------------------"
    print_status "SUCCESS" "Fix-up complete for '$vm_name'."
    print_status "INFO" "Try starting the VM now (Main Menu -> Start a VM)."
    read -p "$(print_status "INPUT" "Press Enter to continue...")"
}
